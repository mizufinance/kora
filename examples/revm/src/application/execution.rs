...
pub(crate) fn precompiles_with_seed(spec: SpecId) -> PrecompilesMap {
    ...
    precompiles.apply_precompile(&address, |_| {
        Some(DynPrecompile::new_stateful(PrecompileId::Custom("commonware_seed".into()), |input| {
    ...
    DB: AlloyDatabase + DatabaseCommit,
{
    ...
    let (db, _) = evm.finish();
    Ok((db, ExecutionOutcome { tx_changes, qmdb_changes }))
}
...
fn account_update_from_evm_account(account: &Account) -> AccountUpdate {
    ...
    let code = account.info.code.as_ref().map(|code| code.original_byte_slice().to_vec());
    let code_hash =
        if account.info.code_hash == B256::ZERO { KECCAK_EMPTY } else { account.info.code_hash };
...
mod tests {
    ...
    fn account_nonce(db: &mut InMemoryDB, address: Address) -> u64 {
        db.basic(address).unwrap().map(|info| info.nonce).unwrap_or(0)
    }
    ...
    fn execute_txs_transfers_value_and_increments_nonce() {
        ...
        assert_eq!(sender_info.balance, U256::from(INITIAL_BALANCE - TRANSFER_AMOUNT));
