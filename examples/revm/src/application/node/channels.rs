...
use super::config::{
    ...
    ChannelReceiver, ChannelSender, Peer,
};
...
pub(super) async fn register_channels(
    ...
    let certs = control.register(CHANNEL_CERTS, quota).await.context("register certs channel")?;
    let resolver =
        control.register(CHANNEL_RESOLVER, quota).await.context("register resolver channel")?;
    let blocks =
        control.register(CHANNEL_BLOCKS, quota).await.context("register blocks channel")?;
    let backfill =
        control.register(CHANNEL_BACKFILL, quota).await.context("register backfill channel")?;

    Ok(NodeChannels { votes, certs, resolver, blocks, backfill })
}
